...
#define BEACON_NO_HOST_TIMEOUT_MS 15000 // Time without host activity to enter beacon mode
...
void beacon_update() {
    ...
    // No point beaconing without a GPS fix
    if (!gps_has_fix) {
        beacon_gate = 4;
        return;
    }
    ...
    // Build JSON payload into temp buffer (shared by encrypted and plain paths)
    char json_buf[256];
