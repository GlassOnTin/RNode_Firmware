...
// RNG callback for mbedTLS
static int beacon_rng(void *ctx, unsigned char *buf, size_t len) {
    (void)ctx;
...
static int beacon_crypto_encrypt(const uint8_t *plaintext, size_t pt_len,
    ...
    // 1. Generate ephemeral X25519 keypair using low-level ECP API
    mbedtls_ecp_group grp;
    mbedtls_mpi d;           // ephemeral private key
    mbedtls_ecp_point Q;     // ephemeral public key
    ...
    ret = mbedtls_ecdh_gen_public(&grp, &d, &Q, beacon_rng, NULL);
    if (ret != 0) goto cleanup_ecdh;

    // Write ephemeral public key (32 bytes, little-endian per RFC 7748)
    size_t olen;
    ret = mbedtls_ecp_point_write_binary(&grp, &Q,
                                          MBEDTLS_ECP_PF_COMPRESSED,
                                          &olen, output, 32);
    if (ret != 0 || olen != 32) { ret = -1; goto cleanup_ecdh; }

    // 2. Load peer's public key and compute ECDH
    ret = mbedtls_ecp_point_read_binary(&grp, &Qp, peer_pub, 32);
    if (ret != 0) goto cleanup_ecdh;

    ret = mbedtls_ecdh_compute_shared(&grp, &z, &Qp, &d, beacon_rng, NULL);
    if (ret != 0) goto cleanup_ecdh;

    // Convert shared secret MPI to bytes.
    // mbedtls_mpi_write_binary outputs big-endian, but Python's
    // X25519.exchange() returns little-endian (RFC 7748). Reverse to match.
    uint8_t ss_be[32];
    ret = mbedtls_mpi_write_binary(&z, ss_be, 32);
    if (ret != 0) goto cleanup_ecdh;

    uint8_t ss_bytes[32];
    for (int i = 0; i < 32; i++) {
        ss_bytes[i] = ss_be[31 - i];
    }
...
cleanup_ecdh:
    ...
    uint8_t derived[64];
    ret = rns_hkdf(ss_bytes, 32, identity_hash, 16, derived);
    if (ret != 0) return -1;
